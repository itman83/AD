...
@interface CNFANewsViewController ()

@end

@implementation CNFANewsViewController
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
      //  self.title = NSLocalizedString(@"报纸", @"报纸");
...
//    back_btn.titleLabel.font=[UIFont fontWithName:@"ZhunYuan" size:13];
...
#pragma Search Bar Right ImageView
 ...
 [searchbg_image addSubview:searchbtn];
    ...
    [tableview registerClass:[CNFANewsCell class] forCellReuseIdentifier:@"identifier"];
...
-(void)fetch_previousTimeStamp_FromDB{
    ...
    NSString *sqlString1 = [NSString stringWithFormat:@"select  * from News_Title"];
   ...
   //Call service for News List
    CNFAGetNewsTitle * gcl = [[CNFAGetNewsTitle alloc] init];
    gcl.delegate = self;
    [gcl callWebService:@"newspapers"];
    ...
    if([searchbar_Txt.text length]>0){
        ...
        if(remoteHostStatus == NotReachable) {
            ...
            [gotosearch setIsallsearch:@"DefaultAllSearch"];
             [gotosearch setFromTab1:@"NewspaperSearch"];
...
-(void)Delete_NewsTitle_Table_DB{
     NSString *insertSQL;
    insertSQL = [NSString stringWithFormat: @"DELETE FROM News_Title"];
...
-(void)InsertRecord_ToNewsTitle_DB{
    ...
    for(int i=0; i<[newsID count];i++){
        insertSQL = [NSString stringWithFormat: @"INSERT INTO News_Title (NewsTitleID,NewsTitleName,NewsTitleLogo,UpdateTimeStamp) VALUES (\"%@\",\"%@\",\"%@\",\"%@\")",[newsID objectAtIndex:i],[newsTitle objectAtIndex:i],[newsTitleimg objectAtIndex:i],[timeStamp objectAtIndex:i]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSLog(@"fetchnewsID count %d::",[fetchnewsID count]);

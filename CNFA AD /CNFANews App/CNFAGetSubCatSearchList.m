...
@implementation CNFAGetSubCatSearchList
@synthesize delegate;
-(void)callWebService:(NSString *)cityID getcateID:(NSString *)catID   getnewsID:(NSString *)NewsID  gettexttext:(NSString *)searchtext{
	...
	NSString * soapMessage = [NSString stringWithFormat:
                              ...
                              "<soap:Body>"
                              "<MediaMenu xmlns=\"http://www.cnfanads.com/webservices/index.php?action=searchads&city_id=%@&cat_id=%@&news_id=%@&data=%@/\" />"
                              "</soap:Body>"
                              "</soap:Envelope>",cityID,catID,NewsID,searchtext];
	
    
    NSURL *  url = [NSURL URLWithString:[NSString stringWithFormat:@"http://www.cnfanads.com/webservices/index.php?action=searchads&city_id=%@&cat_id=%@&news_id=%@&data=%@",cityID,catID,NewsID,searchtext]];
    
    NSLog(@"URL %@",url);
    
    NSMutableURLRequest *theRequest = [NSMutableURLRequest requestWithURL:url];
   	[theRequest addValue: @"text/xml; charset=utf-8" forHTTPHeaderField:@"Content-Type"];
	[theRequest addValue: [NSString stringWithFormat:@"http://www.cnfanads.com/webservices/index.php?action=searchads&city_id=%@&cat_id=%@&news_id=%@&data=%@",cityID,catID,NewsID,searchtext]forHTTPHeaderField:@"SOAPAction"];
...
-(void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error{
    ...
    [delegate getSearchListonSubcatView:Nil getaddcatTitleName:Nil getaddCatText:Nil getaddcatCityname:Nil getadddescNews:Nil getaddCatadddTimeName:Nil getaddCatUpdateTimeStamp:Nil  status:FALSE];
}
...
-(void)connectionDidFinishLoading:(NSURLConnection *)connection{
    ...
    [delegate getSearchListonSubcatView:localsubcatid getaddcatTitleName:localsubcatName getaddCatText:localsubcatText getaddcatCityname:localAdddescCityName getadddescNews:localaddescNewsName  getaddCatadddTimeName:localcreatedTime getaddCatUpdateTimeStamp:localUpdateTme  status:TRUE];

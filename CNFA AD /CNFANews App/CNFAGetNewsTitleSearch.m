...
#import "CNFAGetNewsTitleSearch.h"

@implementation CNFAGetNewsTitleSearch
@synthesize delegate;
-(void)callWebService:(NSString *)Listtype{
	...
	NSString * soapMessage = [NSString stringWithFormat:
                              ...
                              "<soap:Body>"
                              "<MediaMenu xmlns=\"http://www.cnfanads.com/webservices/index.php?search=%@/\" />"
                              "</soap:Body>"
                              "</soap:Envelope>",Listtype];
	
    
    NSURL *  url = [NSURL URLWithString:[NSString stringWithFormat:@"http://www.cnfanads.com/webservices/index.php?search=%@",Listtype]];
    
    NSLog(@"url %@  Listtype %@",url,Listtype);
    
    NSMutableURLRequest *theRequest = [NSMutableURLRequest requestWithURL:url];
   	[theRequest addValue: @"text/xml; charset=utf-8" forHTTPHeaderField:@"Content-Type"];
	[theRequest addValue: [NSString stringWithFormat:@"http://www.cnfanads.com/webservices/index.php?search=%@",Listtype]forHTTPHeaderField:@"SOAPAction"];
...
-(void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error{
	...
	[webData release];
	webData = nil;
    [delegate getNewsTitleListBySearch:Nil getNewsTitle:Nil getImageName:Nil getImageUrl:Nil getTimeStamp:Nil  status:FALSE];
}
...
-(void)connectionDidFinishLoading:(NSURLConnection *)connection{
	[self startParshing:webData];
 	webData = nil;
	[connection release];
    [delegate getNewsTitleListBySearch:localnewsID getNewsTitle:localNewsTitle getImageName:localimageName getImageUrl:localImageURL getTimeStamp:localTimestamp  status:TRUE];

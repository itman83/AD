...
@implementation CNFAGetCatListByNewsandCity
@synthesize delegate;
-(void)callWebService:(NSString *)newsID  getsearchtextID:(NSString *)searchtext{
 ...
 NSString * soapMessage = [NSString stringWithFormat:
                              ...
                              "<soap:Body>"
                              "<MediaMenu xmlns=\"http://www.cnfanads.com/webservices/index.php?action=searchbynewspaper&newspaper_id=%@&data=%@/\" />"
                              "</soap:Body>"
                              "</soap:Envelope>",newsID,searchtext];
	
    
    NSURL *  url = [NSURL URLWithString:[[NSString stringWithFormat:@"http://www.cnfanads.com/webservices/index.php?action=searchbynewspaper&newspaper_id=%@&data=%@",newsID,searchtext] stringByAddingPercentEscapesUsingEncoding: NSUTF8StringEncoding]];
    
    NSLog(@"CNFAGetCatListByNewsandCity URL %@",url);
    
    NSMutableURLRequest *theRequest = [NSMutableURLRequest requestWithURL:url];
   	[theRequest addValue: @"text/xml; charset=utf-8" forHTTPHeaderField:@"Content-Type"];
	[theRequest addValue: [NSString stringWithFormat:@"http://www.cnfanads.com/webservices/index.php?action=searchbynewspaper&newspaper_id=%@&data=%@",newsID,searchtext]forHTTPHeaderField:@"SOAPAction"];
...
-(void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error{
    ...
    [delegate getAddnewsSubtitleByNewsIDsearch:Nil getaddcatTitleName:Nil getaddCatText:Nil getaddcatCityname:Nil getadddescNews:Nil getaddCatadddTimeName:Nil getaddCatUpdateTimeStamp:Nil  status:FALSE];
}
...
-(void)connectionDidFinishLoading:(NSURLConnection *)connection{
    ...
    [delegate getAddnewsSubtitleByNewsIDsearch:localsubcatid getaddcatTitleName:localsubcatName getaddCatText:localsubcatText getaddcatCityname:localAdddescCityName getadddescNews:localaddescNewsName  getaddCatadddTimeName:localcreatedTime getaddCatUpdateTimeStamp:localUpdateTme  status:TRUE];

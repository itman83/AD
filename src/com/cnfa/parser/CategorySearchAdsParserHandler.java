...
public class CategorySearchAdsParserHandler{
//	GetSetModel iModel = null;
//	GetSetModel.ITEM _item;
	public CategorySearchAdsParserHandler(){

	}
	
	public ArrayList<SearchAdsModel> getParseDataByModel(String responce, String newspapername){
		int tagName = 0;
		...
		try {
			...
			while (eventType != XmlPullParser.END_DOCUMENT) {
				if (eventType == XmlPullParser.START_TAG) {
					...
					else if (parser.getName().equals("city_id")) {
				...
				if (eventType == XmlPullParser.END_TAG) {
					if (parser.getName().equals("post")) {
						iModel.setNewspaperName(newspapername);
